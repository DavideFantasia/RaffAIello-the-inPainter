...
INPUT_DIR = f"{CURRENT_DIR}/stendardo/patch_src"       # triplette di: img_XXXX.png, img_XXXX.txt, img_XXXX_mask.png
OUTPUT_DIR = f"{CURRENT_DIR}/stendardo/patch_dst"
WORKFLOW_FILE = f"{CURRENT_DIR}/API-RaffAIello-the-inPainter.json"

# --- ID DEI NODI NEL JSON (Workflow API) ---
...
def main():
    if not os.path.exists(WORKFLOW_FILE):
        print(f"ERRORE: Non trovo il file {WORKFLOW_FILE}. Esportalo da ComfyUI (Save API Format).")
        return
    ...
    for filename in image_files:
        ...
        if comfy_mask_name:
            ...
            workflow[NODE_ID_LOAD_MASK]["inputs"]["image"] = comfy_mask_name
        ...
        if "seed" in workflow[NODE_ID_KSAMPLER]["inputs"]:
            workflow[NODE_ID_KSAMPLER]["inputs"]["seed"] = random.randint(1, 10**14)
        elif "noise_seed" in workflow[NODE_ID_KSAMPLER]["inputs"]:
            workflow[NODE_ID_KSAMPLER]["inputs"]["noise_seed"] = random.randint(1, 10**14)
        ...
        if NODE_ID_SAVE_IMAGE in outputs:
            for item in outputs[NODE_ID_SAVE_IMAGE].get('images', []):
